...
class OrderHandler {
	...
	async getOrderDetails(requestData, headers) {
		...
		const result = await Promise.all([
			QueryHandler.getOrderDetails(sequelize.orders, orderId),
			QueryHandler.getOrderTracking(sequelize.orders, orderId)
		]);

		const orderDetails = result[0];
		const orderTrackingData = result[1].map(data => {
			return {
				order_status: data.status,
				updated_on: moment(data.created_at).format("dddd Do MMM YYYY")
			}
		});
